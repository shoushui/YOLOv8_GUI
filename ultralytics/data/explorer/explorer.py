...
class Explorer:
    ...
    def _yield_batches(self, dataset: ExplorerDataset, data_info: dict, model: YOLO, exclude_keys: List[str]):
        ...
        for i in tqdm(range(len(dataset))):
            ...
            LOGGER.info(f'Processing batch {i + 1}/{len(dataset)} - Progress: {self.progress:.2%}')
            for k in exclude_keys:
                batch.pop(k, None)
            batch = sanitize_batch(batch, data_info)
            batch["vector"] = model.embed(batch["im_file"], verbose=False)[0].detach().tolist()
            LOGGER.info(f'Generated vector for batch {i + 1}: {batch["vector"]}')
            yield [batch]
    ...
    def similarity_index(self, max_dist: float = 0.2, top_k: float = None, force: bool = False) -> Any:  # pd.DataFrame
        ...
        def _yield_sim_idx():
            ...
            for i in tqdm(range(len(embeddings))):
                ...
                LOGGER.info(f'Processing image {i + 1}/{len(embeddings)}: {im_files[i]}')
                yield [
                    {
                        "idx": i,
                        "im_file": im_files[i],
                ...
                LOGGER.info(f'Found {len(sim_idx)} similar images for {im_files[i]} with distances <= {max_dist}.')
