...
@plt_settings()
def plot_results(file="path/to/results.csv", dir="", segment=False, pose=False, classify=False, on_plot=None, save_result_dir=None):
    """
    ...
    ax[1].legend()
    fname = save_dir / "result.png"
    fig.savefig(fname, dpi=200)
    if save_result_dir:
        fname_res = Path(save_result_dir) / "save_model/Loss_PR_MAP.png"
        fig.savefig(fname_res, dpi=200)
    plt.close()
    if on_plot:
        on_plot(fname)
